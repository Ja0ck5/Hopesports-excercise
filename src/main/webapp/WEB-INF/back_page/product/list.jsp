...
function isShow(pageNo,name,brandId,isShow) {
	...
	$("#jvForm").attr("action","isShow.do?pageNo="+pageNo+"&name="+name+"&brandId="+brandId+"&isShow="+isShow).submit();
}
...
名称: <input type="text" value="${name }" name="name"/>
	...
	<select onchange="changePageNo()" name="isShow">
		<option <c:if test="${isShow == 1 }">selected="selected"</c:if> value="1">上架</option>
		<option <c:if test="${isShow == 0 }">selected="selected"</c:if> value="0">下架</option>
	</select>
...
<table cellspacing="1" cellpadding="0" width="100%" border="0" class="pn-ltable">
	...
	<c:forEach items="${pagination.list }" var="entry">
		<tr bgcolor="#ffffff" onmouseover="this.bgColor='#eeeeee'" onmouseout="this.bgColor='#ffffff'">
			...
			<td align="center">
			<a href="#" onclick="window.open('/product/detail.shtml?id=${entry.id}')" class="pn-opt">查看</a> | <a href="#" class="pn-opt">修改</a> | <a href="#" onclick="if(!confirm('您确定删除吗？')) {return false;}" class="pn-opt">删除</a> | <a href="/sku/list.do?productId=${entry.id }&pno=${entry.no}" class="pn-opt">库存</a>
			</td>
		</tr>
	</c:forEach>
	</tbody>
