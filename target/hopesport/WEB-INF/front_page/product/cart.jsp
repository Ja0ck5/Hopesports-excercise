...
function addProductAmount(skuId,buyLimit){
	...
	if(num == buyLimit){
		alert("不能超过："+buyLimit+"件");
		return;
	}
	//$("#num"+skuId).val(++num);
	//请求到后台
	window.location.href='/shopping/buyCart.shtml?skuId='+skuId+'&amount=1&buyLimit='+buyLimit+'&productId='+productId+'';
...
function delProduct(skuId){
	if(confirm("您确定删除?")){
		window.location.href = "/shopping/delItem.shtml?skuId"+skuId;		
	}
}
</script>
</head>
			...
			<div class="page">
				...
				<span class="r box_gray">
					<dl class="total">
						...
						<dd><em class="l">运费：</em>￥<var id="fee">${buyCart.deliverFee }</var>元</dd>
...
<div class="w ofc case" >
	<div class="confirm">
		...
		<div class="ofc pb40" style="text-align: center;height: 200px;margin-top: 80px">
       		 <a href="http://localhost" style="color: red;font-size: 30px;">去首页</a>挑选喜欢的商品
		</div>
	</div>
</div>
</c:if>
