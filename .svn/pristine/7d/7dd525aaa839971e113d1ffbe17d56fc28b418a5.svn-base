...
//登陆
function login(){
	window.location.href = "/shopping/login.shtml?returnUrl=" + window.location.href;//加上当前页面的url
}
//我的订单
function myOrder(){
...
<div class="bar"><div class="bar_w">
	...
	<ul class="r uls">
	<li class="dev">您好,欢迎来到Hopesports！</li>
	<c:if test="${!isLogin }">
		...
		<h2 class="h2 h2_filter mt"><em title="商品筛选">商品筛选</em><cite><a href="javascript:void(0);" id="filterRest" title="重置筛选条件">重置筛选条件</a></cite></h2>
			<ul class="uls filter">
				...
				<li><b>品牌：</b><p>
					...
					<a href="/product/display/list.shtml?brandId=${brand.id }&brandName=${brand.name }&typeName=${typeName }&typeId=${typeId}" title="${brand.name }">${brand.name }</a>
					</c:forEach>
				</p></li>
				</c:if>
				<li><b>价格：</b><p>
				...
				<c:if test="${!empty types && fn:length(types)>0 }">
				<li><b>类型：</b><p>
					<a href="javascript:void(0);" title="不限" class="here">不限</a>
					<c:forEach items="${types }" var="type">
					<a href="/product/display/list.shtml?typeId=${type.id }&typeName=${type.name }&brandId=${brandId }&brandName=${brandName}" title="${type.name }">${type.name }</a>
					</c:forEach>					
				</p></li>
				</c:if>
				<li><b>材质：</b><p>
			...
			<div class="mt ofc">
				...
				<c:forEach items="${pagination.list  }" var="entry">
					<li><!-- 这里不能用 window.location.href，应该使用 window.open('url') -->
						<a href="javascript:void(0)" onclick="window.open('/html/product/${entry.id}.html')" title="${entry.name }" target="_blank" class="pic"><img src="${entry.img.allUrl}" alt="${entry.name }" /></a>
						<dl>
							<!-- dt 10个文字+... -->
