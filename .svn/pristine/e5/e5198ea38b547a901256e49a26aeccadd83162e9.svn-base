...
function changeProvince(code){
	...
	$.post(url,params,function(data){
		...
		var html   = '<option value="" selected>城市</option>';//<option value="" selected>城市</option>
		for(var i=0; i<cities.length;i++){
			html += '<option value="'+ cities[i].code +'" >'+ cities[i].name +'</option>';
			$("#city").html(html);
			$("#town").html('<option value="" selected>县区</option>');
		}	
	},"json");
...
function changeCity(code){
	...
	$.post(url,params,function(data){
		...
		var html   = '<option value="" selected>县区</option>';//<option value="" selected>城市</option>
